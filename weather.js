        ...
        $.get("worldcities.csv",(data)=>{
            ...
            $.each(info,function(i){
                if(finalname === (info[i][1]).toLowerCase()){
                    ...
                    console.log(`X = ${x}, Y = ${y}`);
                    let apikey2 = '1e3101e3cc03399db4a9c7d1043fd62f'
                    let secondAPI = `https://api.openweathermap.org/data/2.5/weather?lat=${x}&lon=${y}&appid=${apikey2}`;
                    ...
                    fetch(secondAPI)
                        .then(response=> response.json())
                            .then(jesresponse => {
                                ...
                                let finaldesc = jesresponse.weather[0].description;
